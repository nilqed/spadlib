...
(PUT 'CONLAW2 'NUMBER-OF-ARGS 2) 
(FLAG '(CONLAW2) 'OPFN) 
(PUT 'CONLAW2 'DEFINED-ON-LINE '37) 
(PUT 'CONLAW2 'DEFINED-IN-FILE 'CRACK/CONLAW2.RED) 
(PUT 'CONLAW2 'PROCEDURE_TYPE '(ARROW (TIMES GENERAL GENERAL) GENERAL)) 
(DE CONLAW2 (PROBLEM RUNMODE)
    (PROG (CONTRACE EQLIST ULIST XLIST DEQU CLLIST DIVLIST SB DENSORD FLIST
           ...
           NON_INT INEQU0 INEQU LOGOOLD TREQLIST FL FACOLD U NODEP SUBL CPU GC
      ...
      (COND
       (LOGOPRINT_
        (PROGN
         ...
         (TERPRI)
         (PROGN
          (PRIN2 "This is CONLAW2 - a program for calculating conservation")
          (PRIN2 " laws of DEs")
          NIL)
         (TERPRI)))
       (T (TERPRI)))
      ...
      (PROG (DENSORD)
       ...
       LAB
        ...
        (PROGN
         ...
         (WHILE (EVALNEQ (AEVAL* H1) (AEVAL* (LIST 'LIST)))
                (PROGN
                 (SETQ H3 (AEVAL* (LIST 'FIRST H2)))
                 (SETQ H5
                         (AEVAL*
                          (LIST 'DIFFERENCE H3
                                (LIST 'QUOTIENT (LIST 'SQFIRST H1)
                                      (LIST 'COEFFN (LIST 'SQFIRST H1) H3
                                            1)))))
                 (SETQ RULES
                         (AEVAL* (LIST 'SQCONS (LIST 'REPLACEBY H3 H5) RULES)))
                 (SETQ E3 (COMBIDIF H3))
                 (SETQ H4 (CONS (LIST (CAR E3) (CDR E3) H5) H4))
                 (SETQ H1 (AEVAL* (LIST 'SQREST H1)))
                 (SETQ H2 (AEVAL* (LIST 'SQREST H2)))
                 (AEVAL* 'NIL)))
         (PROG (E1)
              ...
              (PROGN
               ...
               (PROG ()
                WHILELABEL
                 ...
                 (PROGN
                  (SETQ H3 (COMPAREDIF2 (CAAR H1) (CADAR H1) E1))
                  (COND
                   ((AND (NEQ H3 NIL) (NEQ H3 0))
                    (PROGN
                     ...
                     (PROG (N)
                      ...
                      LAB
                       ...
                       ((LAMBDA (N)
                          (SETQ DEQU
                                  (AEVAL*
                                   (LIST 'TOTDIF DEQU (LIST 'SQPART XLIST N) N
                                         DULIST))))
                        (CAR N))
                       (SETQ N (CDR N))
                       (GO LAB))
                     (SETQ RULES
                             (AEVAL*
                              (LIST 'SQCONS (LIST 'REPLACEBY E1 DEQU) RULES)))
                     (SETQ H1 NIL)))
                   (T (SETQ H1 (CDR H1))))
                  NIL)
                 (GO WHILELABEL))))
                    ...
                    (WHILE (EVALNEQ (AEVAL* H1) (AEVAL* (LIST 'LIST)))
                           (PROGN
                            ...
                            (COND
                             ...
                             (T
                              (PROGN
                               (ASSGNPRI (AEVAL* "Adjoint symmetry:") NIL
                                         'ONLY)
                               (PROGN
                                (ASSGNPRI (AEVAL* "  ( ") NIL 'FIRST)
                                (ASSGNPRI (AEVAL* (LIST 'SQFIRST H3)) NIL NIL)
                               ...
                               (PROGN
                                (PROGN
                                 (PRIN2
                                  "could not be written as a divergence but solves the")
                                 NIL)
                                (TERPRI)
                                (PROGN
                                 (PRIN2
                                  "adjoint symmetry condition and therefore represents")
                                 NIL)
                                (TERPRI)
                                (PROGN (PRIN2 "an adjoint symmetry.") NIL)
                                (TERPRI)
                                NIL)
                               (COND
                                ((AND (EVALNEQ (AEVAL* H8) 0)
                                      (EVALNEQ (AEVAL* CONDI)
                                               (AEVAL* (LIST 'LIST))))
                                 (PROGN
                                  (ASSGNPRI
                                   (AEVAL*
                                    "Please check: if the remaining conditions guarantee ")
                                   NIL 'ONLY)
                                  (PROGN
                                   (ASSGNPRI (AEVAL* " 0 = ") NIL 'FIRST)
                                   (ASSGNPRI (AEVAL* H8) NIL 'LAST))
                                  (PROGN
                                   (ASSGNPRI
                                    (AEVAL*
                                     "then the found characteristic function represents ")
                                    NIL 'FIRST)
                                   (ASSGNPRI (AEVAL* "a conservation law") NIL
                                             'LAST))
                                  (AEVAL* 'NIL)))))))
                            (ASSGNPRI
                             (AEVAL*
      ...
      (COND
       ((BOOLVALUE* (REVALX *TIME))
        (PROGN
         (ASSGNPRI (AEVAL "time to run conlaw2: ") NIL 'FIRST)
